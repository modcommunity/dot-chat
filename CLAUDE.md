...
- **A radius is not a room, and `can_hear_fn` is the seam for the difference.** `can_hear_fn(listener, speaker, listener_at, speaker_at) -> bool` is asked on a `RADIUS` channel after the distance test, only about listeners already inside the radius, and is handed the two positions `position_fn` produced. It exists because game-simple-lobby's wing is a separate room with a 180-thick partition and a 420 radius, so two people either side of the wall were 100 apart and in one conversation. **This addon does not know what a wall is and must not**: what blocks sound is a level's decision, and a wall list here would be a second copy of the level. Unset, the radius is the whole answer. Armed in "router: audience": with the call disabled, three checks fail.
...
# 17 sections, 171 checks, all offline. Exits non-zero on any failure.
```
